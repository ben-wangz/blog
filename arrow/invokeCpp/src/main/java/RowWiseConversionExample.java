package tech.geekcity.arrow.invoke.cpp;
...
public class RowWiseConversionExample {
  ...
  static class data_row {
    ...
    public String toString() {
      return "[id="
          ...
          + cost
          + ",cost_components="
          + Arrays.toString(cost_components)
          + "]";
    }
  }
  ...
  static Status VectorToColumnarTable(data_row[] rows, Table[] table) {
    ...
    for (data_row row : rows) {
      ...
      THROW_ON_FAILURE(
          cost_components_builder.AppendValues(row.cost_components, row.cost_components.length));
    }
  ...
  static Status ColumnarTableToVector(Table table, data_row[][] rows) {
    ...
    Int64Array ids = new Int64Array(table.column(0).chunk(0));
    DoubleArray costs = new DoubleArray(table.column(1).chunk(0));
    ListArray cost_components = new ListArray(table.column(2).chunk(0));
    DoubleArray cost_components_values = new DoubleArray(cost_components.values());
